...
function crear3PartidosMock() {
  const transaccion = db.transaction(() => {
    ...
    for (let i = 0; i < JUGADORES.length; i++) {
      ...
      // Agregar usuario al grupo como jugador
      db.prepare(
        `INSERT INTO UsuariosGrupos (id, grupoId, usuarioId, rol, estaSancionado, fechaIngreso)
         VALUES (?, ?, ?, 'jugador', 0, ?)`
    ...
    // Crear 3 partidos con fechas futuras
    const ahora = new Date();
    const partidas = [
      new Date(ahora.getTime() + 7 * 24 * 60 * 60 * 1000),  // 7 días
      new Date(ahora.getTime() + 14 * 24 * 60 * 60 * 1000), // 14 días
      new Date(ahora.getTime() + 21 * 24 * 60 * 60 * 1000), // 21 días
    ];
    ...
    for (let p = 0; p < partidas.length; p++) {
      ...
      // Agregar 20 jugadores: 14 titulares (7 vs 7), 6 suplentes
      for (let j = 0; j < 20; j++) {
    ...
    console.log(`✓ ${JUGADORES.length} jugadores creados con perfiles completos`);
    console.log(`✓ 3 partidos 7 vs 7 (14 titulares + 6 suplentes) creados con todos los jugadores inscritos`);
  });
